...
def parse_summary_stats(summary_stats_path):
    """File handler to parse  summary_stats into JSON structured format.

    Args:
        summary_stats_path (str): Ruta al archivo summary_stats.tab.

    Returns:
        summary_stats_processed: A dictionary containing consensus data handled.
    """
    # Read the summary_stats.tab file as a DataFrame
    df = pd.read_csv(summary_stats_path, sep="\t")

    # Initialize the dictionary to store the results
    summary_stats_dict = {}

    # Map the columns from the file to the dictionary keys
    column_mapping = {
    ...
    for _, row in df.iterrows():
        ...
        summary_stats_dict[sample_key] = {}

        for key, column in column_mapping.items():
            # Check if the columns type_assignment, subtype_assignment, and clade_assignment are empty
            if key in ["type_assignment", "subtype_assignment", "clade_assignment"]:
                value = row[column] if pd.notna(row[column]) and row[column] != "" else "Not Available NCIT:C126101"
            else:
                value = row[column]

            # Add the value to the dictionary
            summary_stats_dict[sample_key][key] = value
